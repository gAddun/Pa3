...
class NetworkPacket:
    ...
    flag_length = 1
    offset_length = 2
    header_length = dst_addr_S_length + flag_length + offset_length
    ...
    def from_byte_S(self, byte_S, mtu):
        ...
        while True:
            if((self.header_length+len(data_S[offset_size])>mtu)):
                ...
                frag_flag=0
            fragments.append(self(dst_addr, data_S[offset_size:offset_size + mtu - self.header_length], frag_flag, offset_size))
...
class Host:
    ...
    def udt_send(self, dst_addr, data_S, mtu):
        length = len(data_S)
        if(length<=mtu):
            pkt = NetworkPacket(dst_addr, data_S)
        ...
        for i in range(0, len(self.in_intf_L)):
            ...
            try:
                ...
                if pkt_S is not None:
                    ...
                    for p in packets:
                        self.out_intf_L[i].put(p.to_byte_S_frag(), True)
